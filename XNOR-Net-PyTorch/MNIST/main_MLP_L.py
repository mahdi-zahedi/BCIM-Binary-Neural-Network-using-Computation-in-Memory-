...
def test(evaluate=False):
    ...
    for data, target in test_loader:
        ...
        p2=p2.sign()
        p3 = model.bin_FC3_3(x[:,1000:1500])
        p3=p3.sign()
        ...
        for i in range(p1.size(0)):
            for j in range(p1.size(1)):                    
                if ((p1[i][j]>0 and p2[i][j] > 0) or (p1[i][j]>0 and p3[i][j] > 0) or (p3[i][j]>0 and p2[i][j] > 0)):
                    y[i][j]=1
                elif ((p1[i][j]<0 and p2[i][j]<0) or (p1[i][j]<0 and p3[i][j] < 0) or (p3[i][j]<0 and p2[i][j] < 0)):
                    y[i][j]=-1
                else: # never happen 
                    y[i][j]=-1
        #x = p1+p2 + p3
        ...
        p1 = model.bin_FC4_1(x[:,0:500])
        p1=p1.sign()
        p2 = model.bin_FC4_2(x[:,500:1000])
        p2=p2.sign()
