...
def test(evaluate=False):
    ...
    for data, target in test_loader:
        ...
        for i in range(p1.size(0)):
            for j in range(p1.size(1)):                    
                if (p1_P[i][j]>0 and p2_N[i][j] > 0):
                    y[i][j]=1
                elif (p1_N[i][j]>0 and p2_P[i][j] > 0):
                    y[i][j]=1
                elif (p2_0[i][j]>0 and p1_0[i][j] > 0):
                    y[i][j]=1
                else:
                    y[i][j]=-1
        #x = p1+p2
        ...
        #plt.hist(p1.detach().numpy())
        #plt.savefig("plt1.pdf")
        #plt.hist(p2.detach().numpy())
